...
async def hr_planner_node(state: HRState) -> HRState:
    ...
    action_desc = "Synthesizing staffing requirements into HRStaffingPlan"
    if critic_feedback:
        prompt += f"\n\nCRITIC REFINEMENT DIRECTIVE: {critic_feedback}"
        action_desc = "Refining HR plan based on critic feedback"

    model = llm_service.get_model()
    structured_model = model.with_structured_output(HRStaffingPlan)
    
    with logger.status("HR Planner", action_desc):
